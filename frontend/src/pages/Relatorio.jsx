...
export default function Relatorio() {
  ...
  // 🔧 Função calcularResumo (declarada ANTES de ser usada)
  ...
  // 🔧 Função buscar (usa calcularResumo)
  ...
  // 🔧 Carregar dados iniciais (com Promise.all para evitar setState em cascata)
  useEffect(() => {
  ...
  }, [buscar]); // dependência de buscar para recarregar se mudar
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '20px 16px' }}>
        ...
        {movimentacoes.filter(m => m.tipo === 'saida' && m.filial_destino === '').length > 0 && (
          <div style={{ marginBottom: '32px', background: 'var(--cor-superficie)', borderRadius: '12px', padding: '20px', border: '1px solid var(--cor-borda)' }}>
            <h3 style={{ marginBottom: '16px', color: 'var(--cor-texto-titulo)', fontSize: '16px', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <BarChart3 size={18} />
          ...
          <table style={{ minWidth: '800px', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {movimentacoes.length === 0 && !loading && (
                <tr>
                  <td colSpan="8" style={{ padding: '16px', textAlign: 'center', color: 'var(--cor-texto-suave)' }}>
